...
\noindent
This vignette gives an overview of the main functions and tools proposed in the package \pkg{GeneGeneInteR}. Readers that would have more technical details about the statistical methods implemented in the package are encouraged to read the vignette {\bf GenePair}. 
